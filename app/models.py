...
def _to_service_month(value) -> str:
    if value in (None, ""):
...
def _parse_car_services_excel(file_path: str):
    ...
    raw_headers = rows[0]
    header_index = {_normalize_excel_header(name): idx for idx, name in enumerate(raw_headers)}

    def col(*names):
        for name in names:
            normalized = _normalize_excel_header(name)
    ...
    idx_description = col("Описание")
    idx_period = col("Периодичность")
    idx_date = col("Дата", "Дата выполнения")
    idx_mileage = col("Пробег")
    idx_cost = col("Стоимость")

    if idx_name is None:
        raise ValueError("В файле car_services.xlsx обязателен столбец 'Наименование'.")
    ...
    for raw_row in rows[1:]:
        ...
        service_name = str(value_at(idx_name) or "").strip()
        if not service_name:
            continue

        status = str(value_at(idx_status) or "").strip().lower()
        description = str(value_at(idx_description) or "").strip()
        period_type = str(value_at(idx_period) or "").strip() or None
        mileage = _as_int(value_at(idx_mileage), 0)
        cost = _as_float(value_at(idx_cost), 0)

        if status in {"выполнено", "выполнена", "done", "завершено", "завершена"}:
            done_rows.append(
                {
                    ...
                    "mileage": mileage,
                    "service_cost": cost,
                    "service_date": _to_service_month(value_at(idx_date)),
                }
            )
        else:
            normalized_status = "В работе" if status in {"в работе", "в процессе"} else "Планируется"
            planned_rows.append(
                {
                    "service_name": service_name,
                    "detail_description": description,
    ...
    return planned_rows, done_rows


def sync_car_services_from_excel_if_needed(file_path="data/car_services.xlsx"):
    if not os.path.exists(file_path):
        return {"synced": False, "reason": "file_missing"}

    current_mtime = os.path.getmtime(file_path)
    current_mtime_value = f"{current_mtime:.6f}"

    conn = get_connection()
    previous_mtime = _get_meta_value(conn, "car_services_excel_mtime")
    if previous_mtime == current_mtime_value:
        conn.close()
        return {"synced": False, "reason": "not_changed"}

    planned_rows, done_rows = _parse_car_services_excel(file_path)
    ...
    for row in planned_rows:
        conn.execute(
            ...
            INSERT INTO car_planned_services (
                service_name, planned_cost, mileage, detail_description, work_kind, period_type, status
            )
            VALUES (?, ?, ?, ?, ?, ?, ?)
            """,
            (
                ...
                row["planned_cost"],
                row["mileage"],
                row["detail_description"] or None,
                "Разовая",
                row["period_type"],
                row["status"],
            ),
    ...
    for row in done_rows:
        conn.execute(
            ...
            INSERT INTO car_done_services (
                service_name, service_cost, mileage, service_date, detail_description, work_kind, period_type, status
            )
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
    ...
    _set_meta_value(conn, "car_services_excel_mtime", current_mtime_value)
    conn.commit()
    conn.close()
    return {
        "synced": True,
