...
Для `vk-set-config` поддерживаются оба формата флагов: `--token` и `-token` (аналогично для `profile-url` и `timezone`).
...
- `VK API error 15` — access denied / token required. Обычно это не тот тип токена или у токена нет права `messages`.
